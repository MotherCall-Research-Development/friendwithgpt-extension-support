...
function Share(props: { textarea: HTMLTextAreaElement | null }) {
  ...
  const webAccessToggle = (
    <div className="wcg-group wcg-relative wcg-flex">
      ...
      <span
        class="wcg-absolute wcg-left-1/2 wcg-m-4 wcg-mx-auto -wcg-translate-x-6 wcg-translate-y-3 wcg-rounded-md wcg-bg-gray-800 wcg-p-1 
            wcg-text-xs wcg-text-gray-100 wcg-opacity-0 wcg-transition-opacity group-hover:wcg-opacity-100"
      >
        Alt+W
      </span>
    </div>
      ...
      <div className="wcg-toolbar wcg-flex wcg-items-center wcg-gap-2 wcg-rounded-md wcg-px-1">
        ...
        <div class="wcg-scrollbar-hidden wcg-flex wcg-items-center wcg-justify-between wcg-gap-2 wcg-overflow-x-scroll wcg-px-1 lg:wcg-overflow-x-hidden">
          ...
          <Dropdown
            ...
            options={prompts
              .map((prompt) => ({ value: prompt.uuid, label: prompt.name }))
              .concat({
